<script>
	...
	import {
		...
		ScreenSizePx,
		EditorViews
	} from '../globals/globalconstants.js';
	import {
	...
	function updateMainPanelFromEditor(
		globalComponentCollectionStoreVariable,
		globalEditorPreferencesStoreVariable
	) {
		try {
	...
	function setSelectedElement(targetUuid) {
		...
		if (previousSelectedHtmlElementUuid != '')
			if (
				updateGlobalComponentCollectionStore(
	...
	function openSectionModal() {
		...
		var visible = globalVisibility.default.sectionModal ?? false;
		if (!visible) {
			$globalVisibilityStore.update((store) => {
				store.default.sectionModal = true;
				return store;
			});
		}

		console.log($globalVisibilityStore);
	}
			...
			if (event.data) {
				...
				switch (event.data.action) {
					...
					case 'replaceDroppedElementInside':
						replaceDroppedElementInside(
							$globalComponentCollectionStore,
							event.data.data.thisUuid,
							event.data.data.insideUuid
						);
					...
					case 'updateMainPanelFromEditor':
						updateMainPanelFromEditor(
							event.data.data.componentCollection,
							event.data.data.editorPreferences
						);
						break;
	...
	{#if $globalEditorViewStore == EditorViews.PAGE}
		{#if buildType == 'release'}
			<iframe
				...
				title="Editor"
				class=""
				style="--editorWidth:{$globalEditorPreferencesStore.editorData.editorWidth};"
			/>
		{:else}
			<iframe
				id="editorFrame"
