<script>
	...
	import {
		...
		globalLayersExpansionStatesStore
	...
	import Section from '../../(modules)/layers/section.svelte';
	import Block from '../../(modules)/layers/block.svelte';
	...
	let listOfElementsThatCanHaveChildren = [
		'div',
		'gridcontainer',
		'easysection',
		'section',
			...
			<svelte:self component={child} level={level + 1}>
				<div class="w-full flex flex-row">
					...
					{#if child.type == 'div'}
						<Div
							uuid={child.uuid}
							selected={$globalSelectedElementUuidStore === child.uuid}
							on:click={() => setSelected(child.uuid)}
						/>
					...
					{:else}
						<span
							class="flex flex-row gap-2 align-middle my-2"
							class:text-teal-600={$globalSelectedElementUuidStore === child.uuid}
						>
