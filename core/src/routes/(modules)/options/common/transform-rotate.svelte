<script>
    ...
    */
    ...
    $: loaded, (()=>{
        if(loaded == true) {// && innerLoad == false
            ...
            elementDataLoaded.split(" ").forEach( cls => {
                ...
                if(currentClass.startsWith("rotate")){
                    property = currentClass;
                    sliderValue = parseInt(currentClass.replaceAll("rotate-[", "").replaceAll("deg]",""),0);
                    innerLoad = true;
                    foundClass = true;
                }
    ...
    function updateSliderValue(newValue){
        property = "rotate-[" + newValue.toString() + "deg]";
    }
...
<div class="w-full flex flex-row flex-grow justify-between h-8 align-middle items-center">
    <span title="Utilities for rotating elements with transform.">Rotate</span>
