<script>
    ...
    */
    ...
    $: loaded, (()=>{
        if(loaded == true) {// && innerLoad == false
            ...
            elementDataLoaded.split(" ").forEach( cls => {
                ...
                if(currentClass.startsWith("saturate")){
                    property = currentClass;
                    sliderValue = parseInt((parseFloat(currentClass.replaceAll("saturate-[", "").replaceAll("]",""))*100).toString(),0);
                    innerLoad = true;
                    foundClass = true;
                }
    ...
    $: property, (()=>{
        if(loaded == true) {
            ...
            classInput.split(" ").forEach( cls => {
                ...
                if(cls.trim().startsWith("saturate")){
    ...
    function updateSliderValue(newValue){
        property = "saturate-[" + (newValue/100).toFixed(2).toString() + "]";
    }
...
<div class="w-full flex flex-row flex-grow justify-between h-8 align-middle items-center">
    <span title="Utilities for applying saturation filters to an element.">Saturate</span>
