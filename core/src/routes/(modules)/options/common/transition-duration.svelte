<script>
    ...
    */
    ...
    $: loaded, (()=>{
        if(loaded == true) {// && innerLoad == false
            ...
            elementDataLoaded.split(" ").forEach( cls => {
                ...
                if(currentClass.startsWith("duration")){
                    property = currentClass;
                    textValue = currentClass.replaceAll("duration-[", "").replaceAll("ms]","");
                    innerLoad = true;
                    foundClass = true;
                }
    ...
    $: property, (()=>{
        if(loaded == true) {
            ...
            classInput.split(" ").forEach( cls => {
                ...
                if(cls.trim().startsWith("duration")){
            ...
            if(property != "duration-[0ms]" && property != "duration-[ms]"){ 
    ...
    function updateTextValue(newValue){
        property = "duration-[" + newValue.toString() + "ms]";
    }
...
<div class="w-full flex flex-row flex-grow justify-between h-8 align-middle items-center">
    <span title="Utilities for controlling the duration of CSS transitions.">Transition Duration</span>
