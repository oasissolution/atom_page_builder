<script>
    ...
    */
    ...
    $: loaded, (()=>{
        if(loaded == true) {// && innerLoad == false
            ...
            elementDataLoaded.split(" ").forEach( cls => {
                ...
                if(currentClass.startsWith("scale")){
                    property = currentClass;
                    sliderValue = parseInt((parseFloat(currentClass.replaceAll("scale-[", "").replaceAll("]",""))*100).toString(),0);
                    innerLoad = true;
                    foundClass = true;
                }
    ...
    function updateSliderValue(newValue){
        property = "scale-[" + (newValue/100).toFixed(2).toString() + "]";
    }
...
<div class="w-full flex flex-row flex-grow justify-between h-8 align-middle items-center">
    <span title="Utilities for scaling elements with transform.">Scale</span>
