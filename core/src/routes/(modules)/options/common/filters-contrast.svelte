<script>
    ...
    */
    ...
    $: loaded, (()=>{
        if(loaded == true) {// && innerLoad == false
            ...
            elementDataLoaded.split(" ").forEach( cls => {
                ...
                if(currentClass.startsWith("contrast")){
                    property = currentClass;
                    sliderValue = parseInt((parseFloat(currentClass.replaceAll("contrast-[", "").replaceAll("]",""))*100).toString(),0);
                    innerLoad = true;
                    foundClass = true;
                }
    ...
    $: property, (()=>{
        if(loaded == true) {
            ...
            classInput.split(" ").forEach( cls => {
                ...
                if(cls.trim().startsWith("contrast")){
    ...
    function updateSliderValue(newValue){
        property = "contrast-[" + (newValue/100).toFixed(2).toString() + "]";
    }
...
<div class="w-full flex flex-row flex-grow justify-between h-8 align-middle items-center">
    <span title="Utilities for applying contrast filters to an element.">Contrast</span>
