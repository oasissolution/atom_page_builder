<script>
    ...
    */
    ...
    $: loaded, (()=>{
        if(loaded == true) {// && innerLoad == false
            ...
            elementDataLoaded.split(" ").forEach( cls => {
                ...
                if(currentClass.startsWith("skew-y")){
                    property = currentClass;
                    sliderValue = parseInt(currentClass.replaceAll("skew-y-[", "").replaceAll("deg]",""),0);
                    innerLoad = true;
                    foundClass = true;
                }
    ...
    function updateSliderValue(newValue){
        property = "skew-y-[" + newValue.toString() + "deg]";
    }
...
<div class="w-full flex flex-row flex-grow justify-between h-8 align-middle items-center">
    <span title="Utilities for skewing elements with transform. Y Axis / Vertically">Skew Y</span>
