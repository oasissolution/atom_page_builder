<script>
	 ...
	 * @property {string} [src] - Specifies image url for the element
	 */
	 ...
	 * List of attributes
	 * @type [
	 *  "accesskey", "class", "contenteditable", "dir", "draggable", "hidden", "id", "lang", "spellcheck", "style", "tabindex", "title", "src
	 * ]
	 */
	export const elAttr = [
		'accesskey',
	...
	$: data,
		(() => {
			if (data !== undefined && bindElement !== undefined) {
				...
				if (data.src !== undefined) bindElement.setAttribute('src', data.src);
	...
	onMount(() => {
		...
		_class = data.class !== undefined ? data.class : '';
		_src = data.src !== undefined ? data.src : '';
	});
